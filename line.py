...
def line_search_reset(binary_img, left_lane, right_line):
    ...
    window_height = np.int_(binary_img.shape[0] / num_windows)
    ...
    for window in range(num_windows):
        ...
        if len(left_window_inds) > min_num_pixel:
            current_leftX = np.int_(np.mean(nonzerox[left_window_inds]))
        if len(right_window_inds) > min_num_pixel:
            current_rightX = np.int_(np.mean(nonzerox[right_window_inds]))
